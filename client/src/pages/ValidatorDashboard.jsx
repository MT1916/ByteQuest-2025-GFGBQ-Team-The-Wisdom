...
import { getPendingReviews, submitVerdict } from '../services/api';
...
const ValidatorDashboard = () => {
    ...
    const currentReview = reviewQueue.find(r => r.id === selectedId);

    const handleVerdict = async (verdict) => {
        if (!currentReview) return;

        try {
            const res = await submitVerdict(currentReview.id, verdict, "Validator decision");
            if (res.success) {
                setStatusMessage({ type: verdict === 'APPROVED' ? 'success' : 'error', text: `Verdict: ${verdict}` });
                // Remove from queue locally
                setReviewQueue(prev => prev.filter(r => r.id !== currentReview.id));
                setSelectedId(null);

                // Select next available
                const remaining = reviewQueue.filter(r => r.id !== currentReview.id);
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans text-gray-800 flex flex-col h-screen overflow-hidden">
            ...
            <main className="flex-1 flex overflow-hidden">
                ...
                <aside className="w-80 bg-white border-r border-gray-200 flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {reviewQueue.length === 0 && (
                            <div className="p-8 text-center text-gray-400 text-sm">No pending items.</div>
                        )}
                        {reviewQueue.map((item) => (
                            <div
                                key={item.id}
                                onClick={() => setSelectedId(item.id)}
                                className={`p-4 border-b border-gray-100 hover:bg-gray-50 cursor-pointer border-l-4 transition-colors ${selectedId === item.id ? 'border-l-primary bg-primary/5' : 'border-l-transparent'}`}
                            >
                ...
                <section className="flex-1 flex flex-col bg-gray-50 h-full overflow-hidden relative">

                    {statusMessage && (
                        <div className={`absolute top-4 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-full shadow-lg font-bold text-white ${statusMessage.type === 'success' ? 'bg-green-600' : 'bg-red-600'}`}>
                            {statusMessage.text}
                        </div>
                    )}
                                ...
                                <div className="max-w-4xl mx-auto space-y-6">
                                    ...
                                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                                        <div className="flex justify-between items-start mb-6">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 text-sm text-gray-500">
                                                    ...
                                                    <span>•</span>
                                                    <span className="font-mono text-xs bg-gray-100 px-1 rounded">{currentReview.hash}</span>
                                                </div>
                                            </div>
                                        ...
                                        <div className="bg-gray-100 border-2 border-dashed border-gray-300 rounded-xl h-64 flex flex-col items-center justify-center text-gray-500 mb-6 relative group cursor-pointer hover:bg-gray-200 transition-colors">
                                            ...
                                            <span className="font-medium">Preview Proof: {currentReview.document}</span>
                                ...
                                <div className="max-w-4xl mx-auto flex items-center justify-between">
                                    ...
                                    <div className="flex gap-4">
                                        <button
                                            ...
                                            className="px-6 py-3 bg-red-50 text-red-700 font-bold rounded-lg hover:bg-red-100 border border-red-200 flex items-center gap-2 transition-colors"
                                        >
                                            <Lock size={18} /> Reject & Freeze Funds
                                        </button>
                                        <button
                                            onClick={() => handleVerdict('APPROVED')}
                                            className="px-8 py-3 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 shadow-lg shadow-green-600/20 flex items-center gap-2 transition-colors"
                                        >
